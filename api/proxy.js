...
export default async function handler(req, res) {
  ...
  // Hace un intento de pedir al backend y parsear como JSON.
  // Devuelve el objeto parseado, o lanza si no es JSON válido.
  const intentar = async () => {
    const respuesta = await fetch(url, { method: 'GET', redirect: 'follow' });
    const texto = await respuesta.text();
    return JSON.parse(texto); // si esto falla, Apps Script devolvió HTML de error en vez de JSON
  };
  ...
  try {
    ...
    try {
      ...
      // Apps Script puede devolver HTML de error si está saturado de pedidos
      // simultáneos. Reintentamos una vez antes de rendirnos.
      json = await intentar();
    }
