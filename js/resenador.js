...
function construirCardArcActivo(p) {
 ...
 return `
  ...
  <button class="btn-outline-danger btn-full" onclick="abrirModalDNF('${p.idPostulacion}', '${c.nombreLibro}', '${c.nombreAutor}')">Abandonar libro (DNF)</button>
</div>
      </div>
    </div>
  `;
...
/**
 * Abre el modal para abandonar una campaña
 */
function abrirModalDNF(idPostulacion, libroTitulo, autorNombre) {
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Abandonar Campaña</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <p><strong>¿Estás seguro que quieres abandonar esta campaña?</strong></p>
          
          <div class="alert alert-info mt-3">
            <p class="mb-2"><strong>Si abandonas la campaña:</strong></p>
            <ul class="mb-0 ps-3">
              <li>El autor será notificado</li>
              <li>El cupo volverá a estar disponible para otro reseñador</li>
              <li><strong>Recuerda que cuentas con 3 DNF sin penalización por mes.</strong></li>
              <li>A partir de ese número afectará tu porcentaje de completion</li>
            </ul>
          </div>

          <label for="motivoDNF" class="form-label mt-4">
            <strong>¿Por qué decidiste abandonar esta lectura?</strong>
            <small class="text-muted d-block">(Este comentario será visible para el autor solamente)</small>
          </label>
          <textarea 
            id="motivoDNF" 
            class="form-control" 
            rows="4" 
            placeholder="Cuéntale al autor por qué decidiste abandonar esta lectura..."
            maxlength="500"
          ></textarea>
  ...
  const textarea = modal.querySelector('#motivoDNF');
  ...
  // Botón confirmar
  modal.querySelector('#btnConfirmarDNF').addEventListener('click', async () => {
    const motivo = textarea.value;
    
    if (!motivo.trim()) {
      alert('Por favor, explica por qué abandonas esta lectura');
      return;
    }

    await confirmarDNF(idPostulacion, motivo);
    bsModal.hide();
    modal.remove();
  });
...
/**
 * Confirma el abandono de la campaña
 */
